...
class Request_Tester(Base):
    ...
    def api_test(self, event, data: dict):
        ...
        if tag == "sakura" and not api_url.endswith("/v1"):
            ...
            auto_complete == True
            and not api_url.endswith("/v1")
            and not api_url.endswith("/v3")
            and not api_url.endswith("/v4")
        ):
            api_url = api_url + "/v1"
        else:
            api_url = api_url
        ...
        for api_key in api_keys:
            ...
            try:
                # 获取回复内容
                content = self.request_for_content(
                    tag, api_url, api_key, api_format, model, messages
                )
        ...
        self.print("")
        self.info(
            f"接口测试结果：共测试 {len(api_keys)} 个接口，成功 {len(success)} 个，失败 {len(failure)} 个 ..."
        )
        if len(failure) > 0:
    ...
    def generate_messages(self, tag: str, api_format: str):
        ...
        elif tag == "anthropic" or (
            tag.startswith("custom_platform_") and api_format == "Anthropic"
        ):
            messages = [
